...
df = pd.read_csv(r"C:\Users\Usuario\OneDrive - SENA\Documentos\complementario-3125033\07- Sesión\ejercicio4\archivo.csv")

# Asegúrate de que no hay valores NaN en las columnas de interés
df_clean = df.dropna(subset=['Framework'])

# Variables
X = np.arange(len(df_clean)).reshape(-1, 1)
y = df_clean['Framework'].values
...
if p_value < 0.05:
    print(f"Existe una relación significativa entre Semana y Framework (p-value = {p_value:.4f}).")
    
    # Graficar los puntos y la línea de regresión
    plt.figure(figsize=(14, 8))  # Aumentar el tamaño de la figura
    plt.scatter(df_clean['Semana'], y, color='fuchsia', s=100, label='Datos')
    plt.plot(df_clean['Semana'], y_pred, color='red', label='Línea de regresión')
    plt.title('Tendencia del Uso de Framework a lo Largo del Tiempo', fontsize=16)
    ...
    print(f"No existe una relación significativa entre Semana y Framework (p-value = {p_value:.4f}).")
